...
interface User {
  ...
  email?: string;                 // Unique email (Optional in mock)
  db_username: string;           // Database username mapping
  role: string;                  // Role Name (Frontend uses string name not ID)
  status: 'active' | 'inactive';
  isSessionBased: boolean;       // Temporary user flag
...
interface ApprovalRequest {
  ...
  targetUser?: string;           // Target User Name
  description: string;
  timestamp: string;
...
#### Created entities not in mock state but good for API reference:
